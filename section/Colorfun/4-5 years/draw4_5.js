window.addEventListener("DOMContentLoaded", () => {
 ...
 document.getElementById("save").addEventListener("click", () => {
  ...
  function drawImageAsOutline(src, x, y) {
    ...
    image.onload = () => {
      ...
      for (let i = 0; i < data.length; i += 4) {
        const isTransparent = data[i + 3] < 128;
      ...
      ctx.putImageData(imageData, x - imgWidth / 2, y - imgHeight / 2);
    };
  }

  document.querySelectorAll(".shape-container").forEach((shape) => {
    shape.addEventListener("dragstart", (e) => {
      e.dataTransfer.setData("text/path-data", e.currentTarget.dataset.path);
  ...
  canvas.addEventListener("drop", (e) => {
    e.preventDefault();
    const pathData = e.dataTransfer.getData("text/path-data");
    const { x, y } = getMousePos(e);
    ...
    if (pathData) {
      ...
      // Style with a bit more width than pencil
      ctx.lineWidth = 4;
